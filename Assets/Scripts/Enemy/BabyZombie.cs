...
namespace Enemy
{
	public class BabyZombie : EnemyBehaviour
	{
		...
		{
			base.Update();
			if(groundHandler.IsGrounded) GetComponent<Rigidbody2D>().velocity = new Vector3(0, 0, 0); // disable the slip-n-slide
		}
    ...
    public class BabyZombie : EnemyBehaviour
    {
         ...
         */
        {
			...
			if(groundHandler.IsGrounded){
                GetComponent<Rigidbody2D>().velocity = new Vector3(0, 0, 0); // disable the slip-n-slide
                flying = false;
            }
            if (IsPlayerInRange())
            {
         ...
         */
        {
            ...
            {
                if(!flying) transform.position += GetPlayerDirection() * Time.deltaTime * runSpeed;
                else GetPlayerDirection();
            }
            else
            {
                //FaceDirection();
